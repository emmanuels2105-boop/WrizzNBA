...
import type { ConfidenceLevel, PointsPrediction } from "@/lib/db";
...
// Each row is labeled from its own player's team's perspective (team vs
// opponent), which flips order between the two players of the same game --
// normalize so both sides collapse to one filter option instead of two.
function gameKey(p: PointsPrediction): string {
  return `${p.gameDate}|${[p.team, p.opponent].sort().join("-")}`;
}

export function PredictionsTable({ predictions }: { predictions: PointsPrediction[] }) {
  const [selectedGame, setSelectedGame] = useState<string | null>(null);

  const games = useMemo(() => {
    const seen = new Map<string, { key: string; team: string; opponent: string }>();
    for (const p of predictions) {
      const key = gameKey(p);
      if (!seen.has(key)) seen.set(key, { key, team: p.team, opponent: p.opponent });
    }
    return [...seen.values()];
  }, [predictions]);
  ...
  const filtered =
    selectedGame === null ? predictions : predictions.filter((p) => gameKey(p) === selectedGame);
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {games.map((g) => (
          <Button
            key={g.key}
            size="sm"
            variant={selectedGame === g.key ? "default" : "outline"}
        ...
        <span className="ml-auto text-xs text-muted-foreground">
          {filtered.length} of {predictions.length} players
        </span>
      </div>
      ...
      <Table>
        ...
        <TableBody>
          {filtered.map((prediction) => (
            <TableRow key={`${prediction.playerName}-${prediction.gameDate}-${prediction.opponent}`}>
              <TableCell className="font-medium text-foreground">{prediction.playerName}</TableCell>
              <TableCell className="text-muted-foreground">
                <TeamBadge abbreviation={prediction.team} fullName={prediction.teamFullName} />
                <span className="mx-1.5">vs</span>
                <TeamBadge abbreviation={prediction.opponent} fullName={prediction.opponentFullName} />
              </TableCell>
              <TableCell className="text-muted-foreground">{formatGameDate(prediction.gameDate)}</TableCell>
