...
test("decimal odds convert to payout and implied probability correctly", () => {
  assert.ok(Math.abs(decimalOddsToPayoutMultiplier(1.91) - 0.91) < 1e-9);
  assert.ok(Math.abs(decimalOddsToPayoutMultiplier(2.5) - 1.5) < 1e-9);

  assert.ok(Math.abs(decimalOddsToImpliedProbability(1.91) - 1 / 1.91) < 1e-9);
  assert.ok(Math.abs(decimalOddsToImpliedProbability(2.5) - 0.4) < 1e-9);
});

test("EV is ~0 at a price's own breakeven probability, positive above it, negative below it", () => {
  const odds = 1.91;
  const breakeven = decimalOddsToImpliedProbability(odds);
...
test("a line equal to the mean splits ~50/50 regardless of stdev", () => {
  const { over, under } = evaluateLine({ mean: 15, stdev: 4, line: 15, overOdds: 1.91, underOdds: 1.91 });
...
test("a non-zero mean/line gap with a small stdev pushes the probability toward certainty", () => {
  const { over, under } = evaluateLine({ mean: 20, stdev: 1, line: 15, overOdds: 1.91, underOdds: 1.91 });
...
test("a non-positive stdev is treated as a degenerate distribution, not a division error", () => {
  const above = evaluateLine({ mean: 20, stdev: 0, line: 15, overOdds: 1.91, underOdds: 1.91 });
  assert.equal(above.over.probability, 1);
  assert.equal(above.under.probability, 0);

  const below = evaluateLine({ mean: 10, stdev: 0, line: 15, overOdds: 1.91, underOdds: 1.91 });
...
test("over and under probabilities always sum to 1", () => {
  const { over, under } = evaluateLine({ mean: 12.3, stdev: 3.7, line: 14, overOdds: 2.2, underOdds: 1.65 });
